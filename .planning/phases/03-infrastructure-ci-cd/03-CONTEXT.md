...
- **D-08:** Federated credential design: **two explicit per-trigger credentials** on the GitHub Actions service principal:
  ...
  **Amendment (2026-05-12, Gap 8.D resolution):** D-08 RG-scoped Contributor, never subscription rule has ONE named exception: the GHA SP also holds **`Cost Management Contributor` at subscription scope**. Rationale: `azurerm_consumption_budget_subscription.prod` (the EUR 10/mo budget per D-18) is structurally subscription-scoped; no RG-scoped equivalent exists for subscription-wide cost coverage. Cost Management built-in roles cannot create / modify / delete any workload resource: they only operate on `Microsoft.Consumption/*` and `Microsoft.CostManagement/*` providers, so the SP cannot use this role to escalate into compute, data, or identity surfaces. The widening is documented in `infra/modules/identity/main.tf` as `azurerm_role_assignment.gha_cost_management_contributor` with an explanatory description and a link back to this amendment. Verified `pass` in UAT Test 8 run #12 (2026-05-12). Future contributors: any additional subscription-scoped role MUST land here as a separate named exception with the same cannot-mutate-workloads justification, or D-08 must be re-litigated.

  **Amendment v2 (2026-05-12):** D-08's principle is "no workload mutation outside prod RG." Read access is a separate security concern (information disclosure only) and was conflated in the original decision. Per Microsoft's standard CI/CD principal pattern, the GHA SP now holds `Reader` at subscription scope for cross-scope refresh capability. Reader is `*/read` only: no mutation, no role grants, no data-plane access, no secret reads. Combined with the existing narrow Contributor/Officer roles, the SP has the minimum permissions to operate as a Terraform-managed CI principal without violating D-08's mutation-isolation intent.

  Why this resolves the gaps Amendment v1 left open: Cost Management Contributor (granted in v1) does NOT include `Microsoft.Authorization/roleAssignments/read`, so the SP couldn't refresh its own sub-scoped role assignment. Reader covers this. The same applies to `gha_tfstate_blob_data_contributor` whose scope is in `jobrag-tfstate-rg`.

  See `infra/modules/identity/main.tf::azurerm_role_assignment.gha_reader_subscription` and quick task `260512-hui` SUMMARY for the implementation.
