...
- [ ] **CORP-01**: Extraction prompt tightened to reject soft-skill noise (communication, problem-solving, analytical thinking, time management, teamwork, and similar)
- [ ] **CORP-02**: `SkillCategory` enum added to `JobRequirement` (hard / soft / domain); extraction tags every requirement
- [ ] **CORP-03**: Structured `Location` Pydantic schema added (country ISO-3166 code, city, region, remote_allowed boolean) replacing free-text `location` on `job_posting_db`
...
- Unmapped: 67 ⚠️ (roadmapper will resolve)
