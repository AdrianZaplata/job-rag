...
must_haves:
  truths:
    - "03-SMOKE.md exists at .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md (NOT in .planning/quick/)"
    - "File has 13 sections — one per M1–M13 — each with the canonical fields: Behavior, Requirement, Threat, Status, Command, Output, Notes"
    - "Every Status field is one of {PASS, FAIL, PARTIAL, DEFERRED} — no `{PASS|FAIL}` placeholders remain"
    - "Summary table at the top lists all 13 M-IDs with one-line Status + Requirement columns"
    - "Sign-off section lists DEPL-01..DEPL-12 explicitly with one-line coverage notes; each maps to at least one PASS M-section"
    - "Sign-off section lists T-3-01..T-3-08 explicitly with one-line coverage notes; each maps to at least one M-section with mitigation evidence"
    - "Smoke executed timestamp matches the UAT.md evidence dates (executed 2026-05-05..2026-05-19; post-Gap-closure state captured 2026-05-19)"
    - "Cross-link to 03-UAT.md is present (this SMOKE doc is the canonical-format projection of the UAT evidence)"
  artifacts:
    - path: ".planning/phases/03-infrastructure-ci-cd/03-SMOKE.md"
      provides: "M1–M13 canonical-format smoke evidence; closes Plan 03-07 contract via Path A (file actually created)"
      contains: "M1, M2, M3, M4, M5, M6, M7, M8, M9, M10, M11, M12, M13, Sign-off"
      min_lines: 200
  key_links:
    - from: ".planning/phases/03-infrastructure-ci-cd/03-SMOKE.md"
      to: ".planning/phases/03-infrastructure-ci-cd/03-VALIDATION.md lines 70-89"
      via: "M-IDs match exactly (M1, M2, ..., M13)"
      pattern: "^### M[0-9]+:"
    - from: ".planning/phases/03-infrastructure-ci-cd/03-SMOKE.md"
      to: ".planning/phases/03-infrastructure-ci-cd/03-UAT.md"
      via: "Each M section's evidence is a faithful projection of the corresponding UAT Test row"
      pattern: "03-UAT.md"
    - from: ".planning/phases/03-infrastructure-ci-cd/03-SMOKE.md Sign-off section"
      to: "12 DEPL-* requirement IDs + 8 T-3-* threat IDs"
      via: "Explicit roll-call with coverage cross-reference"
      pattern: "DEPL-0[1-9]|DEPL-1[0-2]|T-3-0[1-8]"
---

<objective>
Reshape the 18 PASS rows already captured in `.planning/phases/03-infrastructure-ci-cd/03-UAT.md` into the canonical 13-section `03-SMOKE.md` format defined by Plan 03-07 Task 2 template.

This is a **pure documentation transcription** — the smoke was already executed live against the prod stack across 2026-05-05..2026-05-19, with all evidence in UAT.md format. No new commands run. No `az`, `terraform`, `psql`, `curl`, `gh` invocations. The executor reads the existing UAT rows and projects them into the canonical M{N} section format Plan 03-07 mandates.

Why this exists: Plan 03-07's contract is to produce `03-SMOKE.md` with M1–M13 sections. The smoke was instead recorded as a UAT artifact (18 numbered tests, `result: pass|issue`, `notes: |...`) because Adrian executed it iteratively and the UAT format was already in place from Phases 1+2. Plan 03-08 closed the last 2 gaps; the UAT board is now 18 PASS / 0 ISSUE. Path A closure: synthesize the canonical SMOKE.md file from UAT.md without re-running any verification.

Output: a single new file at `.planning/phases/03-infrastructure-ci-cd/03-SMOKE.md` matching Plan 03-07 Task 2's `<action>` template exactly.

Pre-conditions (all already satisfied):
1. Phase 3 prod stack applied (UAT Tests 4–5 PASS).
2. UAT Tests 3–18 all carry `result: pass` with evidence in `notes:` blocks.
3. Plan 03-08 closed Gap 16.A / Gap 10.A / Gap 12.B — Tests 10, 12, 16 now PASS with documented resolutions.
4. VALIDATION.md lines 70-89 define the canonical M1–M13 behavior list.
</objective>
...
<interfaces>
<!-- Canonical M{N} section format (verbatim from 03-07-PLAN.md Task 2 `<action>` template) -->

```markdown
### M{N} — {Behavior phrase from VALIDATION.md}

**Behavior:** {one-line summary of what is being verified}
**Requirement:** DEPL-XX (and DEPL-YY if multi)
**Threat:** T-3-XX (and T-3-YY if multi)
**Status:** PASS | FAIL | PARTIAL | DEFERRED

**Command:**
```bash
{exact command(s) actually run — taken from UAT.md notes blocks}
```

**Output:**
```
{verbatim or summarised output from UAT notes; redact passwords/keys with <REDACTED>}
```

**Notes:** {deviations, follow-ups, gap-closure cross-references}
```

<!-- M-ID → UAT Test Row mapping (the executor's transcription table) -->

| M-ID  | Behavior (VALIDATION.md L70-89)                          | UAT Test # | UAT Status   | Requirement                            | Threat IDs (Plan 07 §threat_model)           |
|-------|----------------------------------------------------------|------------|--------------|----------------------------------------|----------------------------------------------|
| M1    | Bootstrap remote state from clean clone                   | 3 + 4      | PASS         | DEPL-01                                | T-3-01 (state hygiene)                       |
| M2    | terraform apply creates full Azure resource graph         | 5          | PASS         | DEPL-02, DEPL-03, DEPL-04, DEPL-05, DEPL-06, DEPL-07, DEPL-10, DEPL-11 | T-3-03 (Postgres exposed), T-3-04 (GHCR PAT) |
| M3    | Two-pass CORS bootstrap                                   | 6          | PASS         | DEPL-12                                | T-3-08 (CORS bypass)                         |
| M4    | OIDC federated credential — master push trigger           | 7          | PASS         | DEPL-08, DEPL-09                       | T-3-02 (OIDC trust)                          |
| M5    | OIDC federated credential — environment:production        | 8          | PASS         | DEPL-08                                | T-3-02 (OIDC trust)                          |
| M6    | GHCR image push + ACA pull over HTTPS                     | 9          | PASS         | DEPL-07                                | T-3-04 (GHCR PAT)                            |
| M7    | KV secret resolution via managed identity                 | 10         | PASS (post-08)| DEPL-04                                | T-3-01 (state hygiene)                       |
| M8    | pgvector extension exists in jobrag DB                    | 11         | PASS         | DEPL-04, DEPL-05, DEPL-06              | T-3-03 (Postgres exposed)                    |
| M9    | LAW daily quota + ConsoleLogs only                        | 12         | PASS (post-08)| DEPL-10                                | T-3-06 (Cost / DoS)                          |
| M10   | Budget alert email arrives                                | 13         | PASS         | DEPL-11                                | T-3-06 (Cost / DoS)                          |
| M11   | SSE flow survives Envoy 240s + grace period               | 14         | PASS         | DEPL-07 (D-15)                         | (Phase 1 hand-off — D-15)                    |
| M12   | CIAM authority `.well-known/openid-configuration` reachable| 15         | PASS         | DEPL-08 (D-05 / Phase 4 hand-off)      | T-3-07 (Tenant misconfiguration)             |
| M13   | tfstate has no literal secrets                            | 16         | PASS (post-08)| DEPL-04                                | T-3-01 (state hygiene)                       |

<!-- Sign-off coverage cross-reference (executor builds this table inline) -->

DEPL-01 → M1 (PASS)
DEPL-02 → M2 (PASS) — resource graph
DEPL-03 → M2 (PASS) — ACA env + Container App
DEPL-04 → M2 + M7 + M8 + M13 (all PASS) — KV-backed secrets + MI resolution
DEPL-05 → M2 + M8 (PASS) — Postgres + pgvector
DEPL-06 → M2 + M8 (PASS) — pgvector extension enabled
DEPL-07 → M2 + M6 + M11 (PASS) — image push + ACA pull + revision drain
DEPL-08 → M4 + M5 + M12 (PASS) — OIDC federated credentials + CIAM authority
DEPL-09 → M4 (PASS) — paths-filter contract via deploy-api.yml
DEPL-10 → M2 + M9 (PASS) — LAW daily quota
DEPL-11 → M2 + M10 (PASS) — Budget alert
DEPL-12 → M3 (PASS) — Two-pass CORS

T-3-01 (HIGH, IDis, TF state hygiene) → M13 (PASS, post-08 via value_wo migration)
T-3-02 (HIGH, S, OIDC trust)         → M4 + M5 (PASS)
T-3-03 (HIGH, IDis, Postgres exposed)→ M2 + M8 (PASS — TLS-only + KV-sourced password)
T-3-04 (MED,  IDis, GHCR PAT)        → M6 (PASS — pull works; PAT remains sensitive=true in state)
T-3-05 (MED,  IDis, SWA api_key)     → M5 (PASS — deploy-infra.yml exercised end-to-end)
T-3-06 (MED,  Cost/DoS, Budget)      → M9 + M10 (PASS)
T-3-07 (MED,  S, Tenant misconfig)   → M12 (PASS)
T-3-08 (HIGH, S, CORS bypass)        → M3 (PASS — both allowed and disallowed origins exercised)
</interfaces>
...
<task type="auto">
  <name>Task 1: Write 03-SMOKE.md by projecting UAT.md M-mapped rows into the canonical M{N} section format</name>
  <files>.planning/phases/03-infrastructure-ci-cd/03-SMOKE.md</files>
  <read_first>
    - .planning/phases/03-infrastructure-ci-cd/03-UAT.md (full file — source of truth for every M-section's command + output + notes)
    - .planning/phases/03-infrastructure-ci-cd/03-VALIDATION.md lines 70-89 (canonical M1-M13 behavior phrasing — copy verbatim where possible)
    - .planning/phases/03-infrastructure-ci-cd/03-07-PLAN.md lines 397-488 (Task 2 `<action>` block — exact template structure)
    - .planning/phases/03-infrastructure-ci-cd/03-08-SUMMARY.md (post-08 state for M7/M9/M13)
  </read_first>
  <action>
Create `.planning/phases/03-infrastructure-ci-cd/03-SMOKE.md` (NEW file). The file is the canonical-format projection of `03-UAT.md` evidence into the 13-section M-ID format that Plan 03-07 Task 2 specified.

**SOURCE-OF-TRUTH RULE:** Every command, output, status, and note in 03-SMOKE.md MUST be drawn from the existing `03-UAT.md` `notes: |` blocks. Do NOT invent evidence. Do NOT execute new commands. If 03-UAT.md says "verified transitively" or "deferred to next pass" — preserve that nuance verbatim in the M section's Notes field.

**OUTPUT FILE STRUCTURE (follow exactly):**
...
1. Use the `Write` tool to create `.planning/phases/03-infrastructure-ci-cd/03-SMOKE.md` with the full file content above (the entire block from the leading `# Phase 3 Smoke Evidence (M1–M13)` heading through the final `### Phase-close declaration` block).
2. Do NOT modify any other file. Do NOT touch `03-UAT.md`, `03-07-PLAN.md`, `03-VALIDATION.md`, `STATE.md`, `ROADMAP.md`, or any `infra/**` file.
3. Do NOT run any `az`, `terraform`, `psql`, `curl`, or `gh` command. The smoke is already executed (UAT.md is the evidence). This task is pure documentation transcription.
4. After writing, run the verification commands in `<verify>` to confirm the file passes the structural checks.
  </action>
  <verify>
    <automated>test -f .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md \
      && grep -qE "^## M1 — Bootstrap remote state from clean clone" .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md \
      && grep -qE "^## M13 — tfstate has no literal secrets" .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md \
      && grep -qE "^### Phase-close declaration" .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md \
      && grep -qE "^## Sign-off" .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md \
      && [ "$(grep -cE '^## M[0-9]+ —' .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md)" -eq 13 ] \
      && [ "$(grep -cE '\*\*Status:\*\* (PASS|FAIL|PARTIAL|DEFERRED)' .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md)" -ge 13 ] \
      && ! grep -qE '\{PASS\s*\|\s*FAIL\}|\{PASS\s*\|\s*FAIL\s*\|\s*PARTIAL\}' .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md \
      && [ "$(grep -cE 'DEPL-(0[1-9]|1[0-2])' .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md)" -ge 12 ] \
      && [ "$(grep -cE 'T-3-0[1-8]' .planning/phases/03-infrastructure-ci-cd/03-SMOKE.md)" -ge 8 ]</automated>
  </verify>
  <done>
    1. `.planning/phases/03-infrastructure-ci-cd/03-SMOKE.md` exists.
    2. File contains 13 second-level headings of the form `## M{N} — {Behavior}` (M1 through M13, inclusive).
    3. Every section has a `**Status:**` line set to PASS / FAIL / PARTIAL / DEFERRED — no `{PASS|FAIL}` placeholders left.
    4. Summary table at top lists all 13 M-IDs with Status + Requirement + Threat columns.
    5. Sign-off section lists DEPL-01 through DEPL-12 explicitly with at least one M-section reference per requirement.
    6. Sign-off section lists T-3-01 through T-3-08 explicitly with at least one M-section reference per threat.
    7. Cross-link to `03-UAT.md` is present (source-of-truth statement).
    8. No `infra/**` files modified; no `03-UAT.md` / `STATE.md` / `ROADMAP.md` modified.
    9. The automated verify command exits with code 0.
  </done>
</task>
...
<verification>
- File `.planning/phases/03-infrastructure-ci-cd/03-SMOKE.md` exists.
- Exactly 13 `## M{N} —` headings present (M1–M13).
- Every section's `**Status:**` is a concrete value (PASS/FAIL/PARTIAL/DEFERRED); no template placeholders survive.
- Summary table at top reflects 13 PASS / 0 FAIL / 0 PARTIAL / 0 DEFERRED.
- Sign-off section enumerates DEPL-01..DEPL-12 with coverage map (each requirement → at least one PASS M-section).
- Sign-off section enumerates T-3-01..T-3-08 with coverage map (each threat → at least one M-section).
- No `03-UAT.md`, `03-07-PLAN.md`, `03-VALIDATION.md`, `STATE.md`, `ROADMAP.md`, or `infra/**` file modified.
</verification>

<success_criteria>
1. `03-SMOKE.md` is created at `.planning/phases/03-infrastructure-ci-cd/03-SMOKE.md` (phase artifact location, NOT in `.planning/quick/`).
2. The file is the canonical-format projection of `03-UAT.md` — every command, output, and note traces back to an existing UAT.md row.
3. All 13 M sections carry PASS status (matching the post-Plan-03-08 18/18 PASS UAT state).
4. M7, M9, and M13 sections explicitly reference Plan-03-08 commits (38f06eb, 6e31522, e02b8f0, 09ca58a, a3d18b6) and the 2026-05-19 closure date.
5. M11 section captures the streaming-path PASS (UAT Test 14) and explicitly notes the Phase 4 carry-forward for the Entra JWT gate — but the M11 status itself is PASS because the SSE pipe + drain config (Phase 3's scope) both work; the auth gate is Phase 4's deliverable, not Phase 3's failure.
6. Sign-off section makes both coverage matrices (DEPL-01..12 and T-3-01..08) machine-readable for the verifier.
7. Plan 03-07 contract is closed via Path A (file exists, structure matches the template) without re-running any live verification commands.
</success_criteria>

<output>
After completion, the executor writes `.planning/quick/260519-kr7-synthesize-03-smoke-md-from-existing-03-/260519-kr7-SUMMARY.md` documenting:
- The file created (`03-SMOKE.md`) and its line count.
- Confirmation that the 13 sections / Summary table / Sign-off / cross-references are all in place.
- Note that no source artifacts were modified — pure documentation projection from UAT.md.
- Phase 3 status post-this-quick: still 18/18 PASS UAT; SMOKE.md now also satisfies Plan 03-07's file-existence contract; Phase 3 ready for `/gsd-verify-work 3`.
</output>
