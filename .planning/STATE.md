...
last_updated: "2026-04-28T07:36:44.864Z"
progress:
  total_phases: 8
  completed_phases: 1
  total_plans: 10
...
- `/gsd-plan-phase 2` — create executable plans for Phase 2 (Corpus Cleanup) using `02-CONTEXT.md` as the lock list. Standard granularity → 3-5 plans expected. Likely shape: (1) schema migration + Pydantic + DB models rewrite, (2) prompt + extraction wiring + reextract service, (3) `job-rag reextract` CLI + drift detection + matching/retrieval call-site sweep, (4) validation + the 108-posting re-extraction run.
- Or `/gsd-discuss-phase 3` for Infrastructure & CI/CD (DEPL-01..DEPL-12) — Phases 2 and 3 are parallel-eligible per ROADMAP, so this can run in parallel.
